...
export class UserProfileEditComponent {
  ...
  constructor(private formbuilder: FormBuilder, private apiservice: ApiService, private alertservice: AlertService) {
   ...
   console.log("current user id",this.userid);
    ...
    this.profileEditForm = this.formbuilder.group({
      ...
      newpassword:['',[Validators.required,Validators.minLength(8),Validators.maxLength(25),Validators.pattern("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])[a-zA-Z0-9 \\!\"#\\$%&'\\(\\)\\*\\+,\\-\\.\\/\\:;\\<\\=\\>\\?@\\[\\\\\\]\\^_`\\{\\|\\}~]+$")]],
  ...
  this.apiservice.editUserData(param).subscribe({
    ...
    next: (response: any) => {
      this.alertservice.showSuccess("Details Updated successfully!!!","Success")
    },
    error: (err: any) => {
